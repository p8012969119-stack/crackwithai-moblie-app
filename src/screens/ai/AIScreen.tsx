  ...
  return <SafeAreaView style={s.screen} edges={['top']}>
    <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={s.header}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="Open profile" onPress={() => navigation.navigate('Profile')} style={s.profile}>
          {user?.avatar || (user as any)?.profileImage ? (
            <Image source={{ uri: user?.avatar || (user as any)?.profileImage }} style={s.profileImg} resizeMode="cover" />
          ) : (
            <Image source={require('../../assets/images/logo/crackwithai.png')} style={s.profileLogoImg} resizeMode="contain" />
          )}
        </Pressable>
      </View>
...
const s = StyleSheet.create({
  screen: {flex: 1, backgroundColor: '#FCFBFE'}, header: {paddingHorizontal: 16, paddingVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 10}, brand: {fontSize: 9, letterSpacing: 1.5, color: '#8D7BA9', fontWeight: '700'}, headerTitle: {fontSize: 17, lineHeight: 25, fontWeight: '700', color: '#2F253E'}, iconButton: {minWidth: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center'}, profile: {width: 36, height: 36, borderRadius: 18, backgroundColor: '#FFFFFF', borderWidth: 1.2, borderColor: '#E2E8F0', overflow: 'hidden', alignItems: 'center', justifyContent: 'center'}, profileImg: {width: 36, height: 36, borderRadius: 18}, profileLogoImg: {width: 24, height: 24},
